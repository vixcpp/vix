...
//   -d '{"name":"Jane","age":"22","email":"jane@doe.dev"}' | jq

// # read, update, delete
// curl -s :8080/users/<ID> | jq
// curl -s -X PUT :8080/users/<ID> -H 'content-type: application/json' -d '{"name":"Janet"}' | jq
// curl -s -X DELETE :8080/users/<ID> | jq
