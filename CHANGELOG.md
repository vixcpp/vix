...
`vix build` now exposes diagnostic functionality directly through documented CLI options instead of requiring users to inspect internal files or know hidden environment variables.

Added:

- `--debug` to display structured internal Vix build diagnostics.
- `--debug-log <scope>` with:
  - `cache`
  - `graph`
  - `configure`
  - `process`
...
`--debug` remains separate from raw external output.
...
Captured build logs can now be inspected directly:
...
- `--target native` explicitly selects the current host platform.
- `--targets` now reports detected targets and toolchain availability.
- Native, available, and unavailable targets are distinguished clearly.
- Cross-compilation builds show the selected target when it is relevant.
- Verbose cross builds can show the detected compiler and toolchain information.
- `--sysroot` documentation now makes its cross-toolchain role clearer.
...
CLI options are now the primary user-facing interface while legacy environment-variable compatibility is preserved where necessary.

## Improved

### `vix run` warm-cache performance

Repeated standalone C++ execution is significantly faster when the script and its dependencies have not changed.

Vix now:

- preserves Ninja dependency files for script builds;
- validates dependency modification times correctly across filesystem clock implementations;
- avoids unnecessary CMake and Ninja invocations on valid warm-cache runs;
- reuses previously built script executables when their dependency state remains valid;
- keeps cache fingerprints stable across repeated filesystem timestamp reads.

On a tested warm path, `vix run main.cpp` can execute in roughly tens of milliseconds after the initial build.
...
- Error hints now point users to commands such as:
...
instead of requiring `VIX_LOG_LEVEL=debug` or manual inspection of log files.

## Fixed

### `vix run` stability and caching

This release fixes important standalone C++ execution regressions.

- Standalone C++ scripts no longer enable ASan and UBSan implicitly.
- Vix runtime scripts can use the direct compilation path when the installed Vix libraries satisfy its requirements.
- Generated CMake script builds now use the correct unique CMake target name.
- Script dependency files are preserved so warm-cache validation can correctly detect unchanged dependencies.
- Filesystem modification timestamps are now encoded stably without assuming that `std::filesystem::file_time_type` uses the Unix epoch.
- Touching a source file without changing its contents no longer invalidates the Direct script cache unnecessarily.
...
- Successful builds continue to clean obsolete failure-cache metadata.
- Linker errors preserve missing symbols and provide clearer diagnostics with source locations and actionable hints.
...
v2.8.4 improves the everyday C++ development loop in Vix: faster warm `vix run`, a stable and compact `vix build` interface, clearer diagnostics, and easier-to-discover build and cross-compilation controls.
...
The core identity of v2.7.4 is:

- Composable installed SDK profiles for `vix build`
- Working `web + data` projects without requiring the `all` SDK
- Automatic SDK profile selection from project `vix::*` targets
- A shared CLI source of truth for SDK profile metadata
- More complete exported aliases for SDK consumers
- Clear diagnostics for missing SDK modules and incompatible SDK profile versions
- Regression tests for SDK profile composition and missing-profile diagnostics

## Fixed

### SDK profile composition

Fixed projects that install multiple specialized SDK profiles and then link modules from more than one profile in the same executable.

For example, this workflow now works without installing the full `all` profile:
...
A project can now link both:

```cmake
target_link_libraries(app PRIVATE vix::db vix::websocket)
```

Vix detects that `vix::db` is provided by `data` and `vix::websocket` is provided by `web`, then generates a deterministic composed SDK package for the build.

### CMake package collision between profiles

Fixed the underlying CMake package collision where each SDK profile installed its own `VixConfig.cmake` and `VixTargets.cmake` under the same package name. CMake does not merge multiple `find_package(Vix)` results, so adding both SDK roots to `CMAKE_PREFIX_PATH` was not sufficient.

`vix build` now composes the selected installed profiles into one local `VixConfig.cmake` and passes that composed package through `Vix_DIR`.

### Exported SDK aliases

Recreated public aliases in installed SDK configurations for targets that are aliases in the source tree but exported under implementation names by CMake.
...
### `_IMPORT_PREFIX` rewriting in composed SDKs

Fixed composed SDK target overlays so every `${_IMPORT_PREFIX}` occurrence is rewritten to the source profile installation path. This prevents imported targets from resolving include directories or static libraries as `/include` or `/lib/...` when a target is overlaid from a secondary profile.

### Project-generated targets

Excluded project-generated targets such as `vix::deps` from SDK module resolution. These targets are generated by Vix for a project and must not be treated as modules that an installed SDK profile has to provide.

## Changed

### Shared SDK profile metadata

Moved SDK profile metadata into a shared CLI helper so profile information used by `vix upgrade --sdk info` and profile resolution used by `vix build` come from the same source.

The profile metadata now covers the supported profiles:
...
`vix build` now scans project CMake and Vix manifest files for known `vix::*` targets before CMake configuration. It resolves required modules to installed SDK profiles and refuses to silently mix profiles from different Vix versions.

If only one profile is needed, Vix uses that profile directly. If more than one profile is needed, Vix generates a composed SDK package inside the build directory.

## Diagnostics

### Missing SDK module diagnostics

Improved the error shown when a project requests a known Vix module but the provider SDK profile is missing.

Example with only `data` installed and a project requiring `vix::websocket`:
...
This prevents the generic CMake target error from being the first visible failure for known Vix modules.

### Incompatible SDK versions

Added a strict failure path for mixed installed SDK profile versions. Vix now reports the incompatibility instead of generating a partial composed package.

## Tests

Added SDK composition coverage for:

- `web` profile projects using `vix::websocket`, `vix::requests`, and `vix::middleware`
- `data` profile projects using `vix::db`, `vix::orm`, `vix::cache`, and `vix::kv`
- combined `web + data` projects using `vix::db` and `vix::websocket`
- installation order independence
- separate-profile installation state
- missing SDK provider diagnostics
- incompatible SDK profile versions
- avoiding the generic unresolved CMake target diagnostic for known Vix modules
